...
class MemoryManager:
    ...
    def generate_summary_async(
        self, previous_summary: str, user_message: str, assistant_reply: str
    ) -> None:
        """
        ...
        def _generate():
            ...
            try:
                response = self.bedrock_client.invoke_model(
                    modelId=self.summary_model_id, body=json.dumps(body)
                )
                ...
                if "choices" in result and result["choices"]:
                    ...
                    if isinstance(content, list) and content:
                        ...
                        if isinstance(first, dict):
                            new_summary = first.get("text", "").strip()
                        else:
                            new_summary = str(first).strip()
                ...
                if not new_summary:
                    raise ValueError(
                        f"Unknown summary response format: {list(result.keys())}"
                    )
    ...
    def delete_longterm(self) -> None:
        ...
        if os.path.exists(path):
            os.remove(path)
            print(
                f"[MEMORY] Deleted long-term memory for session {self.session_id[:8]}..."
            )
